...
{
    int A, B, C;
    cout << "\nEnter total items to distribute: ";
    cin >> A;
    cout << "Enter number of positions arranged in a circle: ";
    cin >> B;
    cout << "Enter starting position: ";
    cin >> C;

    int ans = C + A - 1;
    ans = ans % B; // for cyclic process after B comes 1

    if(ans == 0)
        ans = B;

    cout << "\nPosition where the Ath item will be delivered: " << ans;
    cout << endl << endl;    
}